...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/beekeeping'
db=SQLAlchemy(app)
...
class Addabeeproducts(db.Model):
...
@app.route('/beeproducts')
def beeproducts():
    query=db.engine.execute(f"SELECT * FROM `addbeeproducts`") 
    return render_template('beeproducts.html',query=query)

@app.route('/addbeeproduct',methods=['POST','GET'])
@login_required
def addbeeproduct():
    if request.method=="POST":
        ...
        return redirect('/beeproducts')
   
    return render_template('addbeeproducts.html')
...
def edit(rid):
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"UPDATE `register` SET `farmername`='{farmername}',`idnumber`='{idnumber}',`age`='{age}',`gender`='{gender}',`phonenumber`='{phonenumber}',`address`='{address}',`farming`='{farmingtype}'")
...
def register():
    ...
    if request.method=="POST":
        ...
        address=request.form.get('address')
        farmingtype=request.form.get('farmingtype')     
        query=db.engine.execute(f"INSERT INTO `register` (`farmername`,`idnumber`,`age`,`gender`,`phonenumber`,`address`,`farming`) VALUES ('{farmername}','{idnumber}','{age}','{gender}','{phonenumber}','{address}','{farmingtype}')")
